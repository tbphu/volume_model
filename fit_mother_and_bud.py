...
def get_additional_model_parameters(data):
  ...
  budding_start = data_trunc['time'][~np.isnan(data_trunc['bud_V_tot_fl'])][0]
  volume_mother_0 = data_trunc['mother_V_tot_fl'][0]
  volume_bud_0 = data_trunc['bud_V_tot_fl'][~np.isnan(data_trunc['bud_V_tot_fl'])][0]
...
def fit_all_mother_bud(model,
  ...
  for cell_id in len(mothercells_data):
    ...
    fitted_parameters = fit_single_mother_and_bud(model, data, parameters_to_fit, additional_concentrations)
